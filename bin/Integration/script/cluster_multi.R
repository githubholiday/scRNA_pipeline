...
#Marker基因分析
DefaultAssay(immune.combined) <- "RNA"
marker_dir <- paste(outdir,'3_marker',sep='/')
mkdirs(outdir,'3_marker')
setwd(marker_dir)
print(Sys.time())

#
marker_gene_min_pct = as.numeric(ini.list$Para$marker_gene_min.pct)
logfc_threshold = as.numeric(ini.list$Para$marker_gene_logfc.threshold)
test_use = ini.list$Para$marker_gene_test.use


all.markers<-marker_gene(immune.combined,min.pct =marker_gene_min_pct, logfc.threshold = logfc_threshold,outdir=outdir,test.use=test_use, pref=prefix)

immune.combined@misc$markers <- all.markers

saveRDS(immune.combined, file = paste(marker_dir,prefix,"marker.rds",sep='_'))
print(paste("findmarker result save rds :",marker_dir,prefix,"marker.rds",sep='_'))

marker_plot(immune.combined,all.markers,outdir=marker_dir,pref=prefix)
print(Sys.time())
