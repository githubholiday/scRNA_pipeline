#前言：本脚本用于进行单个样本的细胞过滤、细胞群分群、细胞群亚群细分、细胞群marker基因分析
...
rds <- FindVariableFeatures(rds, selection.method = "vst", nfeatures = nfeatures)
rds <- ScaleData(rds)
rds <- RunPCA(rds,features= VariableFeatures(object = rds))
rds <- RunTSNE(rds, reduction = "pca",dims = 1:dims)
rds <- RunUMAP(rds, reduction = "pca", dims = 1:dims)

#rds <- JackStraw(rds, num.replicate = 100)
#rds <- ScoreJackStraw(rds, dims = 1:20)
...
p2 <- DimPlot(tmp, reduction = "tsne", label = T,group.by = "seurat_clusters",label.size = 4,pt.size = 0)+ggtitle('')
