...
npcs = as.integer(ini.list$Para$npcs)
umap_dim = as.integer(ini.list$Para$umap_dim)
pie_size = as.numeric(ini.list$Para$pie_size)
...
pdf( paste( prefix , "_spatial_cluster_umap.pdf" , sep = '') , width = 20 , height = 8)
p1 = DimPlot( stdata, group.by = seurat_clusters , reduction = "umap", label = TRUE ,repel = T , pt.size = 1, label.size = 6)
p2 = SpatialDimPlot( stdata, label = TRUE)
p1 + p2
dev.off()

# 三、单细胞数据情况描述
Idents(scdata)=scdata@meta.data[,celltype]
if(is.null(scdata@reductions$umap)){
	print("RNA数据集中不存在umap slot，需要运行以下步骤：")
	sc = Seurat::ScaleData(scdata,verbose = FALSE) %>% Seurat::RunPCA(., npcs = npcs , verbose = FALSE) %>% Seurat::RunUMAP(., dims = 1:umap_dim , verbose = FALSE)
}else{
	print("RNA数据集中存在umap slot，不在重新分析")
	sc = scdata
}
...
p1 = plotSpatialScatterpie(
    ...
    pie_scale = pie_size) +    # 控制每一个饼的大小，数值变大容易叠在一起
    scale_fill_manual(
        values = pal,
        breaks = names(pal))
...
ggsave(paste(prefix,"_SpatialScatterpie.pdf",sep=''),p1)
ggsave(paste(prefix,"_SpatialScatterpie_nobackground.pdf",sep=''),p3)
...
out = get_spatial_interaction_graph(res[[1]])
dat1 = out$dat1
graph_ntw = out$network
...
dev.off()
write.table(dat1 , file=paste(prefix,"_Interactions.xls",sep='') , sep="\t",quote=F,row.names=F)
