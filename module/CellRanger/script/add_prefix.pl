...
sub Read_DIR{
	...
	if ($#files > 3 or ( -d "$parent_dir/$files[2]")){ ## 目录含一个文件及以上 或只包含一个目录
		for (@files){ ## 遍历目录
			my $file = $_;
			next if ($file =~ /^\./);
...
sub PREFIX_DIR{
	...
	print "Link :$parent_dir/$file $outdir/$new_file\n";
	system("ln -snf $parent_dir/$file $outdir/$new_file");
}
...
sub PREFIX_FILE{
	...
	print "Link :$file_old $outdir/$file_new\n";
	system("ln -snf $file_old $outdir/$file_new");
