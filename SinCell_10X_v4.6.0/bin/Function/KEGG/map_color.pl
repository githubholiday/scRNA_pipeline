...
while(<KEGG_REPORT>){
	chomp;
	my (@up_K_numbers,@down_K_numbers, @K_numbers,%count_of_up_down_gene_in_K);

	my ($mapid,$up_genes,$up_gene_count,$down_genes,$down_gene_count) = (split /\t/, $_)[0,11,12,13,14];
	#print "$mapid\t$up_genes\t$up_gene_count\t$down_genes\t$down_gene_count\n";die;
	...
	foreach my $key ( keys %count_of_up_down_gene_in_K ) { # judge K_numer is up-regulation or down-regulation or both by counts of gene up/down-regulation
		...
		if ( @up_down_type >1 ) {
			my $up_count = $count_of_up_down_gene_in_K{$key}->{up}; 
			my $down_count = $count_of_up_down_gene_in_K{$key}->{down}; 
...
sub get_coords{
	...
	while(<HTML>){
		...
		if ( $_ =~ /\t\t<area id.*?shape=\"rect\"\s+coords=\"(\d+,\d+,\d+,\d+)\".*?title=\"(K\d+[^"]+)\"/ ){
			my $coords = $1;
			#print "$coords\n";
			my $title = $2;
			#print "$title\n";die;
