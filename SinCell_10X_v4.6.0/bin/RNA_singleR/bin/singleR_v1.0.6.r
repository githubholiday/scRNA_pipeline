...
#主要支持多个样品的，列名必须有stim(orig.ident)
#===========================================================
library('getopt')
para<- matrix(c(
	'help',	'h',	0,	"logical",
...
if ( is.null(opt$config) )	{opt$config<-"/nas/data1/bioinfo/PROJECT/Commercial/Cooperation/Database/RNA/SinCell_10X/singleR/current/database/config.ini"}
...
#seurat对象处理以及细胞亚群预测
RG<-function(rds,ref_data,prefix,w_h=c(12,10)){
#	immune.combined <- readRDS(rds)
	#querry <- as_matrix(rds@assays$RNA@data)
	if( length(intersect(c('Spatial'),as.vector(names(rds@assays)))) ==1){querry<-as_matrix(rds@assays$Spatial@data)}else{	querry<-as_matrix(rds@assays$RNA@data)}#rds@assays$Spatial@counts
	print(paste("开始分析，时间为：",Sys.time(),sep=''))
	print(head(ref_data@NAMES))
	...
	####画热图列注释信息
	annotation_col<-data.frame(clusters=rds@meta.data$seurat_clusters,stim=rds@meta.data$orig.ident)
